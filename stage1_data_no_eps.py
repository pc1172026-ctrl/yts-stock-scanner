...
# EPS fallback：若 EPS 統計表缺少個股，再由官方綜合損益表補值。
# 六種產業格式皆讀取，避免金融/證券/保險等公司被漏掉。
TWSE_EPS_FALLBACK = [
...
def _parse_eps_statement_rows(rows, market):
    """從官方綜合損益表讀取累計基本 EPS，作為 EPS 統計表的 fallback。"""
    out = []
    for d in rows:
        sid = _common_code(_first_value(d, ["公司代號", "證券代號", "股票代號"]))
        if not sid:
            continue
        ...
        out.append({
            ...
            "stock_id": sid,
            "name": str(_first_value(d, ["公司名稱", "證券名稱"]) or "").strip(),
            "eps_year": str(_first_value(d, ["年度", "年"]) or "").strip(),
            "eps_quarter": str(_first_value(d, ["季別", "季"]) or "").strip(),
            "eps_cumulative": eps,
            "eps_source": "income_statement",
        })
    return pd.DataFrame(out)


def _fetch_eps_fallback(urls, market, tpex=False):
    frames = []
    errors = []
    for url in urls:
        try:
            rows = _get_json(url, tpex_fallback=tpex)
            df = _parse_eps_statement_rows(rows, market)
            if not df.empty:
                frames.append(df)
        except Exception as e:
...
def fetch_eps_latest():
    ...
    try:
        ...
        if not df.empty:
            df["eps_source"] = "eps_summary"
            primary_frames.append(df)
        report["TWSE_eps_primary"] = len(df)
    except Exception as e:
        report["TWSE_eps_primary_error"] = str(e)
    ...
    try:
        ...
        if not df.empty:
            df["eps_source"] = "eps_summary"
            primary_frames.append(df)
        report["TPEX_eps_primary"] = len(df)
    except Exception as e:
        report["TPEX_eps_primary_error"] = str(e)

    primary = (
        pd.concat(primary_frames, ignore_index=True)
        if primary_frames else
        pd.DataFrame(columns=[
            "market", "stock_id", "name", "eps_year", "eps_quarter",
            "eps_cumulative", "eps_source"
        ])
    )

    tw_fb, tw_err = _fetch_eps_fallback(TWSE_EPS_FALLBACK, "TWSE")
    tp_fb, tp_err = _fetch_eps_fallback(TPEX_EPS_FALLBACK, "TPEX", tpex=True)
    fallback = pd.concat([x for x in [tw_fb, tp_fb] if not x.empty], ignore_index=True) \
        if (not tw_fb.empty or not tp_fb.empty) else pd.DataFrame()

    report["TWSE_eps_fallback"] = len(tw_fb)
    report["TPEX_eps_fallback"] = len(tp_fb)
    if tw_err:
        report["TWSE_eps_fallback_errors"] = tw_err
    if tp_err:
        report["TPEX_eps_fallback_errors"] = tp_err

    # Primary 優先；只有 primary 缺少的股票才使用 fallback。
    if primary.empty:
        x = fallback.copy()
    elif fallback.empty:
        x = primary.copy()
    else:
        keys = set(zip(primary["market"], primary["stock_id"]))
        fb_missing = fallback[
            ~fallback.apply(lambda r: (r["market"], r["stock_id"]) in keys, axis=1)
        ]
        x = pd.concat([primary, fb_missing], ignore_index=True)
    ...
    x = x.drop_duplicates(["market", "stock_id"], keep="first")
    report["EPS_total_after_fallback"] = len(x)
    return x, report
...
def collect_fundamentals():
    ...
    for c in ["revenue_month_yoy", "revenue_cum_yoy"]:
    ...
    # EPS 欄位保留，讓 app.py 不必跟著改；
    # 目前全部標示為人工判斷 / 暫停抓取。
    x["eps_year"] = ""
    x["eps_quarter"] = ""
    x["eps_cumulative"] = np.nan
    x["eps_positive"] = False
    x["eps_source"] = "manual"
    x["eps_status"] = "人工判斷"
    x["eps_data_ok"] = False
